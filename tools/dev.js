...
var templateToolsDir = argv.t;
if (!path.isAbsolute(templateToolsDir)) {
    templateToolsDir = path.join(__dirname, templateToolsDir);
}

// Start webpack-dev-server
const webpack = require('webpack');
const webpackDevServer = require('webpack-dev-server');
const templateWebpackConfig = require(path.join(templateToolsDir, 'webpack.config.js'));
...
const templateCopyFiles = require(path.join(templateToolsDir, 'build.copy.js'));
function docfxBuild(f, curr, previous) {
    ...
    templateCopyFiles();
    execFileSync('docfx', ['build', '-t ' + path.join(templateToolsDir, 'bin')], { stdio: [0, 1, 2] });
}
