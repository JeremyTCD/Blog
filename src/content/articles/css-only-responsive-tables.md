...
display each row as a card when space is tight. This article 
also discusses alternative methods for creating card-style responsive tables, some of
...
must be wrapped. For example, the contents of the following `<td>` elements are wrapped in `<span>` elements:
[!include-code] { 
    "src": "../../resources/articles/cssOnlyResponsiveTables/exampleTableWithWrappers.html",
...
> [!alert-note]
> This is valid HTML, since [\<td\> elements](https://html.spec.whatwg.org/multipage/tables.html#the-td-element) have the 
> [flow content](https://html.spec.whatwg.org/multipage/dom.html#flow-content) content model.  

#### Rationale
Wrapping allows the contents `<td>` elements to be laid out in their own [boxes](https://developer.mozilla.org/en-US/docs/Web/CSS/CSS_Box_Model) 
...
`Persian Cat` would be a text node in the same box that contains the `::before` element. Hacky solutions would be required to lay them out adjacent to 
each other. Some such solutions are discussed in the [alternative methods](http://localhost:8080/articles/css-only-responsive-tables#without-wrappers-in-td-elements) 
section.

### CSS
#### Description
If the non-optional (non-presentational) styles for this method was inlined, the table would look like this in the DOM tree:
[!include-code] { 
...
- No need to set column widths or row heights, since browsers handle [column widths](https://www.w3.org/TR/CSS22/tables.html#auto-table-layout) and 
...
### Notes
#### Accessibility
Accessibility is a problem for responsive tables. The root issue is that table elements (`<table>`, `<tr>`, `<td>`, etc) lose their semantic meaning 
when their `display` properties are changed. For example,
consider the accessibility tree (viewable in Chrome's [accessiblity pane](https://developers.google.com/web/updates/2018/01/devtools#a11y-pane)) of
...
Unfortunately, all CSS-only card-style responsive table methods change `display` properties. That said, when using this method, JS can improve accessibility by applying
[ARIA roles](https://www.w3.org/TR/2017/NOTE-wai-aria-practices-1.1-20171214/examples/table/table.html). For example, consider the card mode table HTML with ARIA attributes:
...
In the HTML for this method, `<div>` elements coupled with `display` properties and ARIA attributes can be used in place of table elements. 
That said, standard table elements offer some conveniences:
...
### Notes
Arbitrary dimensions are often used for such methods to work with tables that have widely varying number of characters each cell. This causes
brittleness. For example,
in the method above, `td:before` has `width: 50%` so that labels do not wrap and overflow the `<td>` element. 
Event then, if a label is longer than `50%` of width, wrapping would still occur.  
...
- it cannot be applied to the example table content used in this article due to the varying number of characters in cells.
Nonetheless, such methods are useful when it isn't possible to wrap the contents of `<td>` elements.

## With Wrappers in \<td\> Elements 
There are a number of methods to create card-style responsive tables when the contents of `<td>` elements are wrapped:
...
# Notes
- The examples in this article are written using plain CSS for simplicity's sake. I'd recommend using [Sass](https://sass-lang.com/) or [Less](http://lesscss.org/) for real world projects. 
...
tables are wider than they were before (despite window width decreasing). Critically, table width is not linearly correlated with window width. This means that setting breakpoints 
according to element widths could offer a better user experience than setting breakpoints according to window width (`@media (min/max-width: <breakpoint>)`).
